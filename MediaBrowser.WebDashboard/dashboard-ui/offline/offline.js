define(["globalize", "connectionManager", "focusManager", "cardBuilder", "emby-itemscontainer", "flexStyles", "scrollStyles"], function(globalize, connectionManager, focusManager, cardBuilder) {
    ...
    return function(view, params) {
        function mergeInto(list1, list2) {
            for (var i = 0, length = list2.length; i < length; i++) list1.push(list2[i])
        }
        ...
        function getLatestItems(type) {
            ...
            return Promise.all(promises).then(function(responses) {
                for (var items = [], i = 0, length = responses.length; i < length; i++) mergeInto(items, responses[i]);
                return items
            })
        }
        ...
        function loadLatestSection(section) {
            return getLatestItems(section.getAttribute("data-mediatype")).then(function(items) {
                return cardBuilder.buildCards(items, {
        ...
        function loadLatest() {
            for (var sections = view.querySelectorAll(".latestSection"), promises = [], i = 0, length = sections.length; i < length; i++) promises.push(loadLatestSection(sections[i]));
        ...
        function renderLocalFolders(parentElement, items, serverName) {
            ...
            html += '<div id="' + id + '" is="emby-itemscontainer" class="itemsContainer vertical-wrap"></div>', html += "</div>", parentElement.insertAdjacentHTML("beforeend", html), cardBuilder.buildCards(items, {
        ...
        function loadServerFolders(parentElement, apiClient) {
            return apiClient.getLocalFolders().then(function(items) {
                return items.length && renderLocalFolders(parentElement, items, apiClient.serverName()), Promise.resolve()
            })
        }
        ...
        function loadOfflineCategories() {
            var promises = [];
            return promises.push(loadLatest()), promises.push(loadAllServerFolders()), Promise.all(promises)
        }
        ...
        view.addEventListener("viewshow", function(e) {
            ...
            isRestored || loadOfflineCategories().then(autoFocus)
        })
    }
});