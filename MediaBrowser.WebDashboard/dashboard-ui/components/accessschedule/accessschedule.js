define(["dialogHelper", "datetime", "emby-select", "paper-icon-button-light", "formDialogStyle"], function(dialogHelper, datetime) {
    ...
    function submitSchedule(context, options) {
        ...
        return parseFloat(updatedSchedule.StartHour) >= parseFloat(updatedSchedule.EndHour) ? void alert(Globalize.translate("ErrorMessageStartHourGreaterThanEnd")) : (context.submitted = !0, options.schedule = Object.assign(options.schedule, updatedSchedule), void dialogHelper.close(context))
    }
    return {
