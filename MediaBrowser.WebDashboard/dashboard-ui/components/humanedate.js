define(["datetime"], function(datetime) {
    ...
    function humane_date(date_str) {
        ...
        for (seconds < 0 && (seconds = Math.abs(seconds)); format = time_formats[i++];)
            if (seconds < format[0]) return 2 == format.length ? format[1] + token : Math.round(seconds / format[2]) + " " + format[1] + token;
        return seconds > 47304e5 ? Math.round(seconds / 47304e5) + " centuries" + token : date_str
    }
