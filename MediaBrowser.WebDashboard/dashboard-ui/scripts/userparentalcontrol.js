define(["jQuery", "datetime", "loading", "listViewStyle", "paper-icon-button-light"], function($, datetime, loading) {
    ...
    function loadUnratedItems(page, user) {
        var items = [{
                ...
                name: Globalize.translate("OptionBlockOthers"),
                value: "Other"
            }],
            html = "";
        ...
        for (var i = 0, length = items.length; i < length; i++) {
            var item = items[i],
                checkedAttribute = user.Policy.BlockUnratedItems.indexOf(item.value) != -1 ? ' checked="checked"' : "";
    ...
    function loadUser(page, user, allParentalRatings) {
        LibraryMenu.setTitle(user.Name), loadUnratedItems(page, user), loadBlockedTags(page, user.Policy.BlockedTags), populateRatings(allParentalRatings, page);
        var ratingValue = "";
    ...
    function showSchedulePopup(page, schedule, index) {
        schedule = schedule || {}, require(["components/accessschedule/accessschedule"], function(accessschedule) {
            accessschedule.show({
                ...
                index == -1 && (index = schedules.length), schedules[index] = updatedSchedule, renderAccessSchedule(page, schedules)
            })
        })
    }
    ...
    function showBlockedTagPopup(page) {
        require(["prompt"], function(prompt) {
            prompt({
                ...
                tags.indexOf(value) == -1 && (tags.push(value), loadBlockedTags(page, tags))
            })
        })
    }
