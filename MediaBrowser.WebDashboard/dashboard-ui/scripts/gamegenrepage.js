define(["jQuery", "imageLoader", "loading"], function($, imageLoader, loading) {
    ...
    function reloadItems(page) {
        loading.show(), ApiClient.getGameGenres(Dashboard.getCurrentUserId(), query).then(function(result) {
            window.scrollTo(0, 0);
            var html = "";
    ...
    $(document).on("pagebeforeshow", "#gameGenresPage", function() {
        ...
        limit != query.Limit && (query.Limit = limit, query.StartIndex = 0), LibraryBrowser.loadSavedQueryValues(getSavedQueryKey(), query), reloadItems(this)
    })
});