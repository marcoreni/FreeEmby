define(["jQuery", "loading", "humanedate", "paper-icon-button-light", "cardStyle"], function($, loading) {
    ...
    function renderUsersIntoElement(elem, users, addConnectIndicator) {
        ...
        elem.html(html), $(".btnUserMenu", elem).on("click", function() {
            showUserMenu(this)
        })
    }
    ...
    function renderUsers(page, users) {
        renderUsersIntoElement($(".localUsers", page), users.filter(function(u) {
            return "Guest" != u.ConnectLinkType
        }), !0), renderUsersIntoElement($(".connectUsers", page), users.filter(function(u) {
            return "Guest" == u.ConnectLinkType
        }))
    }
    ...
    function showInvitePopup(page) {
        Dashboard.getCurrentUser().then(function(user) {
            return user.ConnectUserId ? void require(["components/guestinviter/guestinviter"], function(guestinviter) {
                guestinviter.show().then(function() {
                    loadData(page)
                })
            }) : void showLinkUser(page, user.Id)
        })
    }
