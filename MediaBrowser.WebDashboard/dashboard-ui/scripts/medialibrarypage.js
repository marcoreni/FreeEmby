define(["jQuery", "apphost", "scripts/taskbutton", "loading", "cardStyle"], function($, appHost, taskButton, loading) {
    ...
    function getCollectionTypeOptions() {
        return [{
            ...
            value: "books",
            message: Globalize.translate("BookLibraryHelp")
        }, {
            ...
            name: Globalize.translate("FolderTypePhotos"),
            value: "photos"
        }, {
    ...
    function getVirtualFolderHtml(page, virtualFolder, index) {
        var html = "",
            style = "";
        page.classList.contains("wizardPage") && (style += "min-width:33.3%;"), html += '<div class="card backdropCard scalableCard backdropCard-scalable" style="' + style + '" data-index="' + index + '">', html += '<div class="cardBox visualCardBox">', html += '<div class="cardScalable visualCardBox-cardScalable">', html += '<div class="cardPadder cardPadder-backdrop"></div>', html += '<div class="cardContent">';
        var imgUrl = "";
        if (virtualFolder.PrimaryImageItemId && (imgUrl = ApiClient.getScaledImageUrl(virtualFolder.PrimaryImageItemId, {
                type: "Primary"
            })), imgUrl ? html += '<div class="cardImageContainer editLibrary" style="cursor:pointer;background-image:url(\'' + imgUrl + "');\"></div>" : virtualFolder.showNameWithIcon || (html += '<div class="cardImageContainer editLibrary" style="cursor:pointer;">', html += '<i class="cardImageIcon md-icon" style="color:#444;">' + (virtualFolder.icon || getIcon(virtualFolder.CollectionType)) + "</i>", html += "</div>"), !imgUrl && virtualFolder.showNameWithIcon && (html += '<h1 class="cardImageContainer addLibrary" style="position:absolute;top:0;left:0;right:0;bottom:0;cursor:pointer;flex-direction:column;">', html += '<i class="cardImageIcon md-icon" style="font-size:240%;color:#888;height:auto;width:auto;">' + (virtualFolder.icon || getIcon(virtualFolder.CollectionType)) + "</i>", virtualFolder.showNameWithIcon && (html += '<div style="margin:1em 0;position:width:100%;font-weight:500;color:#444;">', html += virtualFolder.Name, html += "</div>"), html += "</h1>"), html += "</div>", html += "</div>", html += '<div class="cardFooter visualCardBox-cardFooter">', virtualFolder.showMenu !== !1) {
            var moreIcon = "dots-horiz" == appHost.moreIcon ? "&#xE5D3;" : "&#xE5D4;";
            html += '<div style="text-align:right; float:right;padding-top:5px;">', html += '<button type="button" is="paper-icon-button-light" class="btnCardMenu autoSize"><i class="md-icon">' + moreIcon + "</i></button>", html += "</div>"
        }
        html += "<div class='cardText'>", html += virtualFolder.showNameWithIcon ? "&nbsp;" : virtualFolder.Name, html += "</div>";
        var typeName = getCollectionTypeOptions().filter(function(t) {
            return t.value == virtualFolder.CollectionType
        })[0];
        return typeName = typeName ? typeName.name : Globalize.translate("FolderTypeUnset"), html += "<div class='cardText'>", html += virtualFolder.showType === !1 ? "&nbsp;" : typeName, html += "</div>", virtualFolder.showLocations === !1 ? (html += "<div class='cardText'>", html += "&nbsp;", html += "</div>") : virtualFolder.Locations.length ? 1 == virtualFolder.Locations.length ? (html += "<div class='cardText'>", html += virtualFolder.Locations[0], html += "</div>") : (html += "<div class='cardText'>", html += Globalize.translate("NumLocationsValue", virtualFolder.Locations.length), html += "</div>") : (html += "<div class='cardText' style='color:#cc3333;'>", html += Globalize.translate("NumLocationsValue", virtualFolder.Locations.length), html += "</div>"), html += "</div>", html += "</div>", html += "</div>"
    }
    ...
    window.WizardLibraryPage = {
        next: function() {
            ...
            apiClient.ajax({
                ...
                loading.hide(), Dashboard.navigate("wizardsettings.html")
            })
        }
