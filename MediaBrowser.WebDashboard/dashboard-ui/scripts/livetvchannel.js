define(["datetime", "listView", "loading"], function(datetime, listView, loading) {
    ...
    function renderPrograms(page, result) {
        for (var html = "", currentItems = [], currentStartDate = null, i = 0, length = result.Items.length; i < length; i++) {
            ...
            currentStartDate && isSameDay(currentStartDate, itemStartDate) || (currentItems.length && (html += "<h1>" + datetime.toLocaleDateString(currentStartDate, {
                weekday: "long",
                month: "long",
                day: "numeric"
            }) + "</h1>", html += '<div is="emby-itemscontainer" class="vertical-list">' + listView.getListViewHtml({
                items: currentItems,
            ...
            }) + "</div>"), currentStartDate = itemStartDate, currentItems = []), currentItems.push(item)
        }
    ...
    function loadPrograms(page, channelId) {
        ApiClient.getLiveTvPrograms({
            ...
            renderPrograms(page, result), loading.hide()
        })
    }
    return {
