define(["datetime", "jQuery", "loading"], function(datetime, $, loading) {
    ...
    function renderKeys(page, keys, users) {
        var rows = keys.map(function(item) {
                var html = "";
                html += "<tr>", html += "<td>", html += '<button data-token="' + item.AccessToken + '" class="btnRevoke" data-mini="true" title="' + Globalize.translate("ButtonRevoke") + '" style="margin:0;">' + Globalize.translate("ButtonRevoke") + "</button>", html += "</td>", html += '<td style="vertical-align:middle;">', html += item.AccessToken, html += "</td>", html += '<td style="vertical-align:middle;">', html += item.AppName || "", html += "</td>", html += '<td style="vertical-align:middle;">', html += item.DeviceName || "", html += "</td>", html += '<td style="vertical-align:middle;">';
                var user = users.filter(function(u) {
                    return u.Id == item.UserId
                })[0];
                user && (html += user.Name), html += "</td>", html += '<td style="vertical-align:middle;">';
            ...
            elem = $(".resultBody", page).html(rows).parents(".tblApiKeys").table("refresh").trigger("create");
