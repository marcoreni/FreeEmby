    ...
    return function(connectionManager) {
        function getFilesToUpload(files, uploadHistory) {
            return files.filter(function(file) {
                return !!file && 0 === uploadHistory.FilesUploaded.filter(function(u) {
        ...
        function uploadNext(files, index, server, apiClient, resolve, reject) {
            ...
            return index >= length ? void resolve() : void uploadFile(files[index], apiClient).then(function() {
            ...
            return new Promise(function(resolve, reject) {
                require(["fileupload", "cryptojs-sha1"], function(FileUpload) {
                    var name = "camera image " + (new Date).getTime(),
                        url = apiClient.getUrl("Devices/CameraUploads", {
                    ...
                    console.log("Uploading file to " + url), (new FileUpload).upload(file, name, url).then(resolve, reject)
                })
            })
        }
        var self = this;
