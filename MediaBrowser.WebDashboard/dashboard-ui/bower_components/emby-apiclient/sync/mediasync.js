define(["localassetmanager"], function(localassetmanager) {
        ...
        return console.log("[mediasync] Begin processDownloadStatus"), localassetmanager.resyncTransfers().then(function() {
            return localassetmanager.getServerItems(serverInfo.Id).then(function(items) {
                ...
                var p = Promise.resolve(),
                    cnt = 0,
                    progressItems = items.filter(function(item) {
                        return "transferring" === item.SyncStatus || "queued" === item.SyncStatus
                    });
                return progressItems.forEach(function(item) {
                    p = p.then(function() {
    ...
    function reportTransfer(apiClient, item) {
        return localassetmanager.getItemFileSize(item.LocalPath).then(function(size) {
            return size > 0 ? apiClient.reportSyncJobItemTransferred(item.SyncJobItemId).then(function() {
                return item.SyncStatus = "synced", localassetmanager.addOrUpdateLocalItem(item)
            }, function(error) {
            ...
            return console.error("[mediasync] reportTransfer: error on getItemFileSize. Deleting item.", error), localassetmanager.removeLocalItem(item).then(function() {
                ...
                return console.log("[mediasync] reportTransfer: Failed to delete item.", error), Promise.resolve()
            })
        })
    }
    ...
    function afterSyncData(apiClient, serverInfo, enableSyncUserItemAccess, syncDataResult) {
        ...
        }), enableSyncUserItemAccess && (p = p.then(function() {
    ...
    function getNewMedia(apiClient, serverInfo, options, downloadCount) {
        return console.log("[mediasync] Begin getNewMedia"), apiClient.getReadySyncItems(apiClient.deviceId()).then(function(jobItems) {
            ...
            return jobItems.forEach(function(jobItem) {
                currentCount++ <= maxDownloads && (p = p.then(function() {
    ...
    function getNewItem(jobItem, apiClient, serverInfo, options) {
        ...
        return localassetmanager.getLocalItem(serverInfo.Id, libraryItem.Id).then(function(existingItem) {
            return !existingItem || "queued" !== existingItem.SyncStatus && "transferring" !== existingItem.SyncStatus && "synced" !== existingItem.SyncStatus ? (libraryItem.CanDelete = !1, libraryItem.CanDownload = !1, libraryItem.SupportsSync = !1, libraryItem.People = [], libraryItem.Chapters = [], libraryItem.Studios = [], libraryItem.UserData = {}, libraryItem.SpecialFeatureCount = null, libraryItem.LocalTrailerCount = null, libraryItem.RemoteTrailers = [], localassetmanager.createLocalItem(libraryItem, serverInfo, jobItem).then(function(localItem) {
                return console.log("[mediasync] getNewItem: createLocalItem completed"), localItem.SyncStatus = "queued", downloadParentItems(apiClient, jobItem, localItem, serverInfo, options).then(function() {
            ...
            })) : (console.log("[mediasync] getNewItem: getLocalItem found existing item"), Promise.resolve())
        })
    }
    ...
    function downloadParentItems(apiClient, jobItem, localItem, serverInfo, options) {
        var p = Promise.resolve(),
            ...
            itemType = (libraryItem.Type || "").toLowerCase();
        (libraryItem.ImageTags || {}).Logo;
        switch (itemType) {
            case "episode":
                libraryItem.SeriesId && (p = p.then(function() {
                    return downloadItem(apiClient, libraryItem, libraryItem.SeriesId, serverInfo).then(function(seriesItem) {
                        return libraryItem.SeriesLogoImageTag = (seriesItem.Item.ImageTags || {}).Logo, Promise.resolve()
                    })
    ...
    function downloadMedia(apiClient, jobItem, localItem, options) {
        ...
        return console.log("[mediasync] Downloading media. Url: " + url + ". Local path: " + localPath), options = options || {}, localassetmanager.downloadFile(url, localItem).then(function(filename) {
            return localItem.SyncStatus = "transferring", localassetmanager.addOrUpdateLocalItem(localItem)
        })
    }
    ...
    function getImages(apiClient, jobItem, localItem) {
        ...
        })), libraryItem.SeriesId && libraryItem.SeriesLogoImageTag && (p = p.then(function() {
    ...
    function getItemSubtitle(file, apiClient, jobItem, localItem, mediaSource) {
        ...
        return localassetmanager.downloadSubtitles(url, fileName).then(function(subtitlePath) {
            return localItem.AdditionalFiles && localItem.AdditionalFiles.forEach(function(item) {
                item.Name === file.Name && (item.Path = subtitlePath)
            }), subtitleStream.Path = subtitlePath, localassetmanager.addOrUpdateLocalItem(localItem)
        })
    }
            ...
            return console.log("[mediasync]************************************* Start sync"), checkLocalFileExistence(apiClient, serverInfo, options).then(function() {
                return processDownloadStatus(apiClient, serverInfo, options).then(function() {
                    return localassetmanager.getDownloadItemCount().then(function(downloadCount) {
                        return options.syncCheckProgressOnly === !0 && downloadCount > 2 ? Promise.resolve() : reportOfflineActions(apiClient, serverInfo).then(function() {
                            return getNewMedia(apiClient, serverInfo, options, downloadCount).then(function() {
                                return syncData(apiClient, serverInfo, !1).then(function() {
