define(["events", "apiclient", "appStorage"], function(
  events,
  apiClientFactory,
  appStorage
) {
  ...
  function updateServerInfo(server, systemInfo) {
    (server.Name = systemInfo.ServerName), (server.Id =
      systemInfo.Id), systemInfo.LocalAddress &&
      (server.LocalAddress = systemInfo.LocalAddress), systemInfo.WanAddress &&
      (server.RemoteAddress = systemInfo.WanAddress), systemInfo.MacAddress &&
  ...
  function getFetchPromise(request) {
    ...
    return request.data &&
      ...
      (headers["Content-Type"] = contentType), request.timeout
      ? fetchWithTimeout(request.url, fetchRequest, request.timeout)
  ...
  function fetchWithTimeout(url, options, timeoutMs) {
    return console.log(
      ...
      (options = options || {}), (options.credentials = "same-origin"), fetch(
        url,
        options
      ).then(
        function(response) {
          clearTimeout(timeout), console.log(
  ...
  function ajax(request) {
    ...
    return (request.headers = request.headers || {}), console.log(
      "ConnectionManager requesting url: " + request.url
      ...
      function(err) {
        throw (
          console.log(
            "ConnectionManager request failed to url: " + request.url
          ),
          err
        );
      }
  ...
  function tryConnect(url, timeout) {
    return (url = getEmbyServerUrl(url, "system/info/public")), console.log(
      "tryConnect url: " + url
    ), ajax({
      type: "GET",
      url: url,
    ...
    ConnectionManager = function(
      ...
      function onConnectUserSignIn(user) {
        appStorage.removeItem(
          "lastLocalServerId"
        ), (connectUser = user), events.trigger(self, "connectusersignedin", [
          user
        ]);
      }
      function getOrAddApiClient(server, connectionMode) {
        ...
        if (!apiClient) {
          ...
          (apiClient = new apiClientFactory(
            ...
            deviceId,
            devicePixelRatio
          )), self._apiClients.push(apiClient), apiClient.serverInfo(
            server
      ...
      function onAuthenticated(apiClient, result, options, saveCredentials) {
        ...
        options.updateDateLastAccessed !== !1 &&
          (server.DateLastAccessed = new Date().getTime()), (server.Id =
          result.ServerId), saveCredentials
          ? (
          ...
          credentials.Servers,
          server
        ), saveUserInfoIntoCredentials(
          server,
          result.User
        ), credentialProvider.credentials(credentials), apiClient.serverInfo(
          server
        ), afterConnected(apiClient, options), onLocalUserSignIn(
          server,
      ...
      function saveUserInfoIntoCredentials(server, user) {
        var info = { Id: user.Id, IsSignedInOffline: !0 };
        credentialProvider.addOrUpdateUser(server, info);
      }
      function afterConnected(apiClient, options) {
        (options = options || {}), options.reportCapabilities !== !1 &&
          apiClient.reportCapabilities(
            capabilities
          ), (apiClient.enableAutomaticBitrateDetection =
          options.enableAutomaticBitrateDetection), options.enableWebSocket !==
          !1 &&
          (
      ...
      function onLocalUserSignIn(server, connectionMode, user) {
        ...
        var promise = self.onLocalUserSignedIn
      ...
      function ensureConnectUser(credentials) {
        return connectUser && connectUser.Id === credentials.ConnectUserId
          ? Promise.resolve()
          : credentials.ConnectUserId && credentials.ConnectAccessToken
            ? (
                (connectUser = null),
                getConnectUser(
      ...
      function addAuthenticationInfoFromConnect(
        ...
        var url = ServerInfo.getServerAddress(server, connectionMode);
        url = getEmbyServerUrl(
          url,
          "Connect/Exchange?format=json&ConnectUserId=" +
            credentials.ConnectUserId
        );
        var auth =
          'MediaBrowser Client="' +
        ...
        return ajax({
          ...
          function(auth) {
            return (server.UserId = auth.LocalUserId), (server.AccessToken =
      ...
      function validateAuthentication(server, connectionMode) {
        ...
        return ajax({
          ...
          function(systemInfo) {
            return updateServerInfo(server, systemInfo), server.UserId
              ? ajax({
                  type: "GET",
      ...
      function getImageUrl(localUser) {
        if (connectUser && connectUser.ImageUrl)
          return { url: connectUser.ImageUrl };
        if (localUser && localUser.PrimaryImageTag) {
          ...
          return { url: url, supportsParams: !0 };
        }
        return { url: null, supportsParams: !1 };
      }
      function logoutOfServer(apiClient) {
        var serverInfo = apiClient.serverInfo() || {},
          logoutInfo = { serverId: serverInfo.Id };
      ...
      function filterServers(servers, connectServers) {
        return servers.filter(function(server) {
          return (
            ...
            connectServers.filter(function(connectServer) {
        ...
        return new Promise(function(resolve, reject) {
          var onFinish = function(foundServers) {
            var servers = foundServers.map(function(foundServer) {
              ...
              return (info.LastConnectionMode = info.ManualAddress
                ? ConnectionMode.Manual
                : ConnectionMode.Local), info;
            });
            resolve(servers);
          };
      ...
      function compareVersions(a, b) {
        (a = a.split(".")), (b = b.split("."));
        for (
          var i = 0, length = Math.max(a.length, b.length);
          i < length;
          i++
        ) {
      ...
      function testNextConnectionMode(tests, index, server, options, resolve) {
        ...
        return mode === ConnectionMode.Local
          ? (
              (enableRetry = !0),
              (timeout = 8e3),
              stringEqualsIgnoreCase(address, server.ManualAddress) &&
                (
                  console.log(
                    "skipping LocalAddress test because it is the same as ManualAddress"
                  ),
                  (skipTest = !0)
                )
            )
          : mode === ConnectionMode.Manual &&
            stringEqualsIgnoreCase(address, server.LocalAddress) &&
            ((enableRetry = !0), (timeout = 8e3)), skipTest || !address
          ? (
              console.log("skipping test at index " + index),
              void testNextConnectionMode(
                tests,
                index + 1,
                server,
                options,
          ...
          : (
              console.log(
                "testing connection mode " +
                  mode +
                  " with server " +
                  server.Name
              ),
              void tryConnect(address, timeout).then(
                function(result) {
                  1 === compareVersions(self.minServerVersion(), result.Version)
                    ? (
                        console.log(
                    ...
                    : result.Id !== server.Id
                      ...
                      : (
                          console.log(
                            "calling onSuccessfulConnection with connection mode " +
                              mode +
                              " with server " +
                              server.Name
                          ),
                ...
                function() {
                  console.log(
                    "test failed for connection mode " +
                      mode +
                      " with server " +
                      server.Name
                  ), enableRetry
      ...
      function onSuccessfulConnection(
        ...
        var credentials = credentialProvider.credentials();
        (options = options || {}), credentials.ConnectAccessToken &&
        options.enableAutoLogin !== !1
      ...
      function afterConnectValidated(
        ...
        if (((options = options || {}), options.enableAutoLogin === !1))
          (server.UserId = null), (server.AccessToken = null);
        else if (
          verifyLocalAuthentication &&
          server.AccessToken &&
          options.enableAutoLogin !== !1
        )
          return void validateAuthentication(
            server,
            connectionMode
          ).then(function() {
            afterConnectValidated(
        ...
        updateServerInfo(
          server,
          systemInfo
        ), (server.LastConnectionMode = connectionMode), options.updateDateLastAccessed !==
          !1 &&
          (server.DateLastAccessed = new Date().getTime()), credentialProvider.addOrUpdateServer(
          credentials.Servers,
          server
        ), credentialProvider.credentials(credentials);
        var result = { Servers: [] };
        (result.ApiClient = getOrAddApiClient(
          server,
          connectionMode
        )), result.ApiClient.setSystemInfo(systemInfo), (result.State =
          server.AccessToken && options.enableAutoLogin !== !1
            ? ConnectionState.SignedIn
            : ConnectionState.ServerSignIn), result.Servers.push(
          server
        ), result.ApiClient.updateServerInfo(
          server,
          connectionMode
        ), result.State === ConnectionState.SignedIn &&
          afterConnected(result.ApiClient, options), resolve(
          result
      ...
      function normalizeAddress(address) {
        return (address = address.trim()), 0 !==
          address.toLowerCase().indexOf("http") &&
          (address = "http://" + address), (address = replaceAll(
          address,
          "Http:",
          "http:"
      ...
      this._apiClients = [];
      var connectUser;
      return (self.connectUser = function() {
        return connectUser;
      }), (self._minServerVersion = "3.2.5"), (self.appVersion = function() {
        return appVersion;
        ...
        var existingServers = credentialProvider
            .credentials()
        ...
        events.trigger(self, "apiclientcreated", [
          apiClient
        ]), existingServer.Id ||
          apiClient.getPublicSystemInfo().then(function(systemInfo) {
            var credentials = credentialProvider.credentials();
            (existingServer.Id =
              systemInfo.Id), apiClient.serverInfo(existingServer), (credentials.Servers = [existingServer]), credentialProvider.credentials(credentials);
          });
      }), (self.clearData = function() {
        console.log("connection manager clearing data"), (connectUser = null);
        var credentials = credentialProvider.credentials();
        (credentials.ConnectAccessToken = null), (credentials.ConnectUserId = null), (credentials.Servers = []), credentialProvider.credentials(
          credentials
        );
        ...
        return new Promise(function(resolve, reject) {
          ...
          function onEnsureConnectUserDone() {
            apiClient && apiClient.getCurrentUserId()
        ...
        for (
          var promises = [], i = 0, length = self._apiClients.length;
          i < length;
          i++
        ) {
        ...
        return Promise.all(promises).then(function() {
          for (
            var credentials = credentialProvider.credentials(),
              servers = credentials.Servers.filter(function(u) {
            ...
            for (
              var serverUsers = server.Users || [],
                k = 0,
                numUsers = serverUsers.length;
              k < numUsers;
              k++
            )
              serverUsers[k].IsSignedInOffline = !1;
          }
          credentials.ConnectAccessToken &&
            appStorage.removeItem(
              "lastLocalServerId"
        ...
        return Promise.all([
          ...
          return mergeServers(
            ...
            servers,
            foundServers
          ), mergeServers(credentialProvider, servers, connectServers), (servers = filterServers(servers, connectServers)), servers.sort(
            function(a, b) {
              return (b.DateLastAccessed || 0) - (a.DateLastAccessed || 0);
            }
      ...
      }), (self.getOffineResult = function() {}), (self.connectToServers = function(
        servers,
        options
      ) {
        console.log(
        ...
        if (defaultServer)
          return self
            .connectToServer(defaultServer, options)
            .then(function(result) {
              return result.State === ConnectionState.Unavailable &&
                (result.State =
                  ConnectionState.ServerSelection), console.log("resolving connectToServers with result.State: " + result.State), result;
            });
        var firstServer = servers.length ? servers[0] : null;
        return firstServer
          ? self.connectToServer(firstServer, options).then(function(result) {
              return result.State === ConnectionState.SignedIn
                ? result
                : {
                    Servers: servers,
                    State:
                      servers.length || self.connectUser()
        ...
        return console.log("begin connectToServer"), new Promise(function(
          resolve,
          reject
        ) {
          var tests = [];
          null != server.LastConnectionMode, tests.indexOf(
            ConnectionMode.Manual
          ) === -1 && tests.push(ConnectionMode.Manual), tests.indexOf(
            ConnectionMode.Local
          ) === -1 && tests.push(ConnectionMode.Local), tests.indexOf(
            ConnectionMode.Remote
          ) === -1 && tests.push(ConnectionMode.Remote), (options =
            options || {}), console.log(
        ...
        return new Promise(function(resolve, reject) {
          function onFail() {
            console.log(
              "connectToAddress " + address + " failed"
            ), resolveFailure(self, resolve);
          }
          return address
            ? (
                (address = normalizeAddress(address)),
                void tryConnect(address, defaultTimeout).then(function(
                  publicInfo
                ) {
                  ...
                  updateServerInfo(server, publicInfo), self
        ...
        return username && password
          ? ajax({
              ...
              dataType: "json",
              contentType: "application/x-www-form-urlencoded; charset=UTF-8",
              headers: { "X-Application": appName + "/" + appVersion }
            }).then(function(result) {
              var credentials = credentialProvider.credentials();
              return (credentials.ConnectAccessToken =
                result.AccessToken), (credentials.ConnectUserId = result.User.Id), credentialProvider.credentials(credentials), onConnectUserSignIn(result.User), result;
            })
          : Promise.reject();
        ...
        if (!email) return Promise.reject({ errorCode: "invalidinput" });
        if (!username) return Promise.reject({ errorCode: "invalidinput" });
        if (!password) return Promise.reject({ errorCode: "invalidinput" });
        if (!passwordConfirm)
          return Promise.reject({ errorCode: "passwordmatch" });
        if (password !== passwordConfirm)
          return Promise.reject({ errorCode: "passwordmatch" });
        var data = { email: email, userName: username, rawpw: password };
        return options.grecaptcha &&
          (data.grecaptcha = options.grecaptcha), ajax({
          type: "POST",
        ...
        return (server = server.length
          ...
          var url =
            "https://connect.emby.media/service/serverAuthorizations?serverId=" +
        ...
        if (!self.connectUserId()) throw new Error("null connectUserId");
        var url =
          "https://connect.emby.media/service/ServerAuthorizations/accept?serverId=" +
          serverId +
          "&userId=" +
          self.connectUserId();
        return ajax({
        ...
        var params = {
          ...
          timeSinceLastValidation =
            new Date().getTime() - (regInfo.lastValidDate || 0);
        ...
        regInfo.deviceId &&
          regInfo.deviceId !== params.deviceId &&
          (updateDevicePromise = ajax({
            url:
              "https://mb3admin.com/admin/service/registration/updateDevice?" +
              paramsToString({
        ...
        var onFailure = function(err) {
          if (
            (
              ...
              timeSinceLastValidation <= 6048e5
            )
          )
            return console.log(
              "getRegistrationInfo returning cached info"
        ...
        return updateDevicePromise.then(function() {
          return apiClient.getCurrentUser().then(function(user) {
            return (params.embyUserName =
              user.Name), appStorage.setItem(
                            cacheKey,
                            JSON.stringify({
        ...
        if (!pinInfo) throw new Error("pinInfo cannot be null");
        var queryString = { deviceId: pinInfo.DeviceId, pin: pinInfo.Pin },
          request = {
            type: "GET",
        ...
        return exchangePin(pinInfo).then(function(result) {
          var credentials = credentialProvider.credentials();
          return (credentials.ConnectAccessToken =
            result.AccessToken), (credentials.ConnectUserId = result.UserId), credentialProvider.credentials(credentials), ensureConnectUser(credentials);
        });
      }), self;
    };
  return (ConnectionManager.prototype.getApiClients = function() {
    for (
      var servers = this.getSavedServers(), i = 0, length = servers.length;
      i < length;
      i++
    )
    ...
    return item.ServerId &&
      (item = item.ServerId), this._apiClients.filter(function(a) {
