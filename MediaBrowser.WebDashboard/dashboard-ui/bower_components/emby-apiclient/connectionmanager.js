        ...
        ConnectionManager = function(credentialProvider, appName, appVersion, deviceName, deviceId, capabilities, devicePixelRatio) {
            ...
            function getOrAddApiClient(server, connectionMode) {
                ...
                if (!apiClient) {
                    ...
                    apiClient = new apiClientFactory(url, appName, appVersion, deviceName, deviceId, devicePixelRatio), apiClients.push(apiClient), apiClient.serverInfo(server), apiClient.onAuthenticated = function(instance, result) {
            ...
            function onLocalUserSignIn(server, connectionMode, user) {
                self.connectUserId() ? appStorage.removeItem("lastLocalServerId") : appStorage.setItem("lastLocalServerId", server.Id), getOrAddApiClient(server, connectionMode);
            ...
            function testNextConnectionMode(tests, index, server, options, resolve) {
                if (index >= tests.length) return console.log("Tested all connection modes. Failing server connection."), void resolveFailure(resolve);
            ...
            return self.minServerVersion = function(val) {
                return val && (minServerVersion = val), minServerVersion
                ...
                for (var promises = [], i = 0, length = apiClients.length; i < length; i++) {
                ...
                return Promise.all([getConnectServers(credentials), findServers()]).then(function(responses) {
                    var connectServers = responses[0],
                        ...
                        servers = credentials.Servers.slice(0);
                    return mergeServers(servers, foundServers), mergeServers(servers, connectServers), servers = filterServers(servers, connectServers), servers.sort(function(a, b) {
                        return (b.DateLastAccessed || 0) - (a.DateLastAccessed || 0)
                    }), credentials.Servers = servers, credentialProvider.credentials(credentials), servers
                })
            }, self.connect = function(options) {
                return console.log("Begin connect"), self.getAvailableServers().then(function(servers) {
                    return self.connectToServers(servers, options)
                })
            }, self.getOffineResult = function() {}, self.connectToServers = function(servers, options) {
                ...
                return new Promise(function(resolve, reject) {
                    function onFail() {
                        console.log("connectToAddress " + address + " failed"), resolveFailure(resolve)
                    }
                    return address ? (address = normalizeAddress(address), void tryConnect(address, defaultTimeout).then(function(publicInfo) {
                        console.log("connectToAddress " + address + " succeeded");
                        var server = {
                            ManualAddress: address,
                            LastConnectionMode: ConnectionMode.Manual
                        };
                        updateServerInfo(server, publicInfo), self.connectToServer(server, options).then(resolve, onFail)
                    }, onFail)) : void reject()
                })
                ...
                return item.ServerId && (item = item.ServerId), apiClients.filter(function(a) {
                ...
                return updateDevicePromise.then(function() {
                    return apiClient.getCurrentUser().then(function(user) {
                        return params.embyUserName = user.Name, ajax({
