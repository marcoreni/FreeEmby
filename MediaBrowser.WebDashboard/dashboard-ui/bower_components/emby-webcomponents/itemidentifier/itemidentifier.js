define(["dialogHelper", "loading", "cardBuilder", "connectionManager", "require", "globalize", "scrollHelper", "layoutManager", "focusManager", "emby-input", "emby-checkbox", "paper-icon-button-light", "css!./../formdialog", "material-icons", "cardStyle"], function(dialogHelper, loading, cardBuilder, connectionManager, require, globalize, scrollHelper, layoutManager, focusManager) {
    ...
    function searchForIdentificationResults(page) {
        ...
        for (i = 0, length = identifyField.length; i < length; i++) value = identifyField[i].value, value && ("number" === identifyField[i].type && (value = parseInt(value)), lookupInfo[identifyField[i].getAttribute("data-lookup")] = value);
        var hasId = !1,
    ...
    function getSearchResultHtml(result, index) {
        ...
        } else html += '<div class="cardImageContainer coveredImage ' + cardBuilder.getDefaultColorClass(result.Name) + '"><div class="cardText cardCenteredText">' + result.Name + "</div></div>";
        return html += "</div>", html += "</div>", html += '<div class="cardFooter visualCardBox-cardFooter">', html += '<div class="cardText cardTextCentered">' + result.Name + "</div>", html += '<div class="cardText cardText-secondary cardTextCentered">', html += result.ProductionYear || "&nbsp;", html += "</div>", result.GameSystem && (html += '<div class="cardText cardText-secondary cardTextCentered">', html += result.GameSystem, html += "</div>"), html += "</div>", html += "</div>", html += "</button>"
    }
