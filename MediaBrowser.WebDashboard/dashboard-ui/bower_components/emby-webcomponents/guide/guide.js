define(["require", "browser", "globalize", "connectionManager", "serverNotifications", "loading", "datetime", "focusManager", "userSettings", "imageLoader", "events", "layoutManager", "itemShortcuts", "registrationServices", "dom", "clearButtonStyle", "css!./guide.css", "programStyles", "material-icons", "scrollStyles", "emby-button", "paper-icon-button-light", "emby-tabs", "emby-scroller", "flexStyles"], function(require, browser, globalize, connectionManager, serverNotifications, loading, datetime, focusManager, userSettings, imageLoader, events, layoutManager, itemShortcuts, registrationServices, dom) {
    ...
    function Guide(options) {
        ...
        function normalizeDateToTimeslot(date) {
            ...
            return minutesOffset >= 0 ? date.setHours(date.getHours(), cellCurationMinutes, 0, 0) : date.setHours(date.getHours(), 0, 0, 0), date
        }
        ...
        function startCurrentTimeUpdateInterval() {
            clearCurrentTimeUpdateInterval(), currentTimeUpdateInterval = setInterval(updateCurrentTimeIndicator, 6e4), updateCurrentTimeIndicator()
        }

        function clearCurrentTimeUpdateInterval() {
            var interval = currentTimeUpdateInterval;
            interval && clearInterval(interval), currentTimeUpdateInterval = null, currentTimeIndicatorBar = null, currentTimeIndicatorArrow = null
        }

        function updateCurrentTimeIndicator() {
            if (currentTimeIndicatorBar || (currentTimeIndicatorBar = options.element.querySelector(".currentTimeIndicatorBar")), currentTimeIndicatorArrow || (currentTimeIndicatorArrow = options.element.querySelector(".currentTimeIndicatorArrowContainer")), currentDate) {
                var dateDifference = (new Date).getTime() - currentDate.getTime(),
                    pct = dateDifference > 0 ? dateDifference / totalRendererdMs : 0;
                pct = Math.min(pct, 1), pct <= 0 || pct >= 1 ? (currentTimeIndicatorBar.classList.add("hide"), currentTimeIndicatorArrow.classList.add("hide")) : (currentTimeIndicatorBar.classList.remove("hide"), currentTimeIndicatorArrow.classList.remove("hide"), currentTimeIndicatorBar.style.transform = "scaleX(" + pct + ")", currentTimeIndicatorArrow.style.transform = "translateX(" + 100 * pct + "%)")
            }
        }
        ...
        function getChannelLimit(context) {
            return registrationServices.validateFeature("livetv").then(function() {
                ...
                return context.querySelector(".guideRequiresUnlock").classList.add("hide"), limit
            }, function() {
                var limit = 5;
                return context.querySelector(".guideRequiresUnlock").classList.remove("hide"), context.querySelector(".unlockText").innerHTML = globalize.translate("sharedcomponents#LiveTvGuideRequiresUnlock", limit), limit
            })
        }
        ...
        function reloadGuide(context, newStartDate, scrollToTimeMs, focusToTimeMs, focusProgramOnRender) {
            ...
            channelQuery.UserId = apiClient.getCurrentUserId(), getChannelLimit(context).then(function(channelLimit) {
                currentChannelLimit = channelLimit, showLoading(), channelQuery.StartIndex = currentStartIndex, channelQuery.Limit = channelLimit, channelQuery.AddCurrentProgram = !1, channelQuery.EnableUserData = !1, channelQuery.EnableImageTypes = "Primary";
                var categories = self.categoryOptions.categories || [],
                    displayMovieContent = !categories.length || categories.indexOf("movies") !== -1,
                    displaySportsContent = !categories.length || categories.indexOf("sports") !== -1,
                    displayNewsContent = !categories.length || categories.indexOf("news") !== -1,
                    displayKidsContent = !categories.length || categories.indexOf("kids") !== -1,
                    displaySeriesContent = !categories.length || categories.indexOf("series") !== -1;
                ...
                console.log(nextDay), apiClient.getLiveTvChannels(channelQuery).then(function(channelsResult) {
                    ...
                    channelsResult.TotalRecordCount > channelLimit ? (context.querySelector(".guideOptions").classList.remove("hide"), btnPreviousPage.classList.remove("hide"), btnNextPage.classList.remove("hide"), channelQuery.StartIndex ? context.querySelector(".btnPreviousPage").disabled = !1 : context.querySelector(".btnPreviousPage").disabled = !0, channelQuery.StartIndex + channelLimit < channelsResult.TotalRecordCount ? btnNextPage.disabled = !1 : btnNextPage.disabled = !0) : context.querySelector(".guideOptions").classList.add("hide"), apiClient.getLiveTvPrograms({
                        ...
                        renderGuide(context, date, channelsResult.Items, programsResult.Items, apiClient, scrollToTimeMs, focusToTimeMs, focusProgramOnRender), hideLoading()
                    })
                })
            })
        }
        ...
        function getTimeslotHeadersHtml(startDate, endDateTime) {
            ...
            return html += '<div class="currentTimeIndicatorBar hide">', html += "</div>", html += '<div class="currentTimeIndicatorArrowContainer hide">', html += '<i class="currentTimeIndicatorArrow md-icon">arrow_drop_down</i>', html += "</div>"
        }
        ...
        function getChannelProgramsHtml(context, date, channel, programs, options, listInfo) {
            var html = "",
                ...
                outerCssClass = layoutManager.tv ? "channelPrograms channelPrograms-tv" : "channelPrograms";
            ...
            for (var programsFound, clickAction = layoutManager.tv ? "link" : "programdialog", categories = self.categoryOptions.categories || [], displayMovieContent = !categories.length || categories.indexOf("movies") !== -1, displaySportsContent = !categories.length || categories.indexOf("sports") !== -1, displayNewsContent = !categories.length || categories.indexOf("news") !== -1, displayKidsContent = !categories.length || categories.indexOf("kids") !== -1, displaySeriesContent = !categories.length || categories.indexOf("series") !== -1, enableColorCodedBackgrounds = "true" === userSettings.get("guide-colorcodedbackgrounds"), i = listInfo.startIndex, length = programs.length; i < length; i++) {
                ...
                if (program.ChannelId === channel.Id) {
                    if (programsFound = !0, listInfo.startIndex++, parseDates(program), !(program.EndDateLocal.getTime() < startMs)) {
                        ...
                        var renderStartMs = Math.max(program.StartDateLocal.getTime(), startMs),
                            startPercent = (program.StartDateLocal.getTime() - startMs) / msPerDay;
        ...
        function renderChannelHeaders(context, channels, apiClient) {
            for (var html = "", i = 0, length = channels.length; i < length; i++) {
                ...
                layoutManager.tv && (cssClass += " channelHeaderCell-tv"), html += '<button type="button" class="' + cssClass + '"' + dataSrc + ' data-action="link" data-isfolder="' + channel.IsFolder + '" data-id="' + channel.Id + '" data-serverid="' + channel.ServerId + '" data-type="' + channel.Type + '">', channel.Number && (html += '<h3 class="guideChannelNumber">' + channel.Number + "</h3>"), !hasChannelImage && channel.Name && (html += '<div class="guideChannelName">' + channel.Name + "</div>"), html += "</button>"
            }
        ...
        function renderPrograms(context, date, channels, programs) {
            for (var allowIndicators = dom.getWindowSize().innerWidth >= 600, options = {
                    ...
                    showNewIndicator: allowIndicators && "true" === userSettings.get("guide-indicator-new"),
        ...
        function getProgramSortOrder(program, channels) {
            ...
            var start = datetime.parseISO8601Date(program.StartDate, {
                toLocal: !0
            });
            return 1e7 * channelIndex + start.getTime() / 6e4
        }
        ...
        function renderGuide(context, date, channels, programs, apiClient, scrollToTimeMs, focusToTimeMs, focusProgramOnRender) {
            ...
            context.querySelector(".timeslotHeaders").innerHTML = getTimeslotHeadersHtml(startDate, endDate), startCurrentTimeUpdateInterval(), items = {}, renderPrograms(context, date, channels, programs), focusProgramOnRender ? focusProgram(context, itemId, channelRowId, focusToTimeMs) : scrollProgramGridToTimeMs(context, scrollToTimeMs)
        }
        ...
        function focusProgram(context, itemId, channelRowId, focusToTimeMs) {
            ...
            if (itemId && (focusElem = context.querySelector('[data-id="' + itemId + '"]')), focusElem) focusManager.focus(focusElem);
                ...
                var autoFocusParent, programGrid = context.querySelector(".programGrid");
                channelRowId && (autoFocusParent = context.querySelector('[data-channelid="' + channelRowId + '"]')), autoFocusParent || (autoFocusParent = programGrid);
        ...
        function onProgramGridScroll(context, elem, timeslotHeaders) {
            (new Date).getTime() - lastHeaderScroll >= 1e3 && (lastGridScroll = (new Date).getTime(), nativeScrollTo(timeslotHeaders, elem.scrollLeft, !0))
        }
        ...
        function changeDate(page, date, scrollToTimeMs, focusToTimeMs, focusProgramOnRender) {
            ...
            currentDate = newStartDate, reloadGuide(page, newStartDate, scrollToTimeMs, focusToTimeMs, focusProgramOnRender)
        }
        ...
        function setDateRange(page, guideInfo) {
            ...
            start.setHours(0, 0, 0, 0), end.setHours(0, 0, 0, 0), start.getTime() >= end.getTime() && end.setDate(start.getDate() + 1), start = new Date(Math.max(today, start));
            var dateTabsHtml = "",
                tabIndex = 0,
                date = new Date;
            for (currentDate && date.setTime(currentDate.getTime()), date.setHours(0, 0, 0, 0), start.setHours(0, 0, 0, 0); start <= end;) {
                var isActive = date.getDate() === start.getDate() && date.getMonth() === start.getMonth() && date.getFullYear() === start.getFullYear();
                dateTabsHtml += getDateTabText(start, isActive, tabIndex), start.setDate(start.getDate() + 1), start.setHours(0, 0, 0, 0), tabIndex++
            }
            page.querySelector(".emby-tabs-slider").innerHTML = dateTabsHtml, page.querySelector(".guideDateTabs").refresh();
            var newDate = new Date,
                scrollToTimeMs = 60 * newDate.getHours() * 60 * 1e3,
                minutes = newDate.getMinutes();
            minutes >= 30 && (scrollToTimeMs += 18e5);
            var focusToTimeMs = 60 * (60 * newDate.getHours() + minutes) * 1e3;
        ...
        function setScrollEvents(view, enabled) {
            layoutManager.tv && require(["scrollHelper"], function(scrollHelper) {
                var fn = enabled ? "on" : "off";
                scrollHelper.centerFocus[fn](view.querySelector(".programGrid"), !0)
            })
        }
        ...
        function onTimerCreated(e, apiClient, data) {
            for (var programId = data.ProgramId, newTimerId = data.Id, cells = options.element.querySelectorAll('.programCell[data-id="' + programId + '"]'), i = 0, length = cells.length; i < length; i++) {
                ...
                icon || cell.insertAdjacentHTML("beforeend", '<i class="timerIcon md-icon programIcon">&#xE061;</i>'), newTimerId && cell.setAttribute("data-timerid", newTimerId)
            }
        }
        ...
        var currentDate, autoRefreshInterval, cellCurationMinutes = 30,
            cellDurationMs = 60 * cellCurationMinutes * 1e3,
            msPerDay = 864e5,
            totalRendererdMs = msPerDay,
            currentStartIndex = 0,
        ...
        self.refresh = function() {
            ...
            stopAutoRefresh(), events.off(serverNotifications, "TimerCreated", onTimerCreated), events.off(serverNotifications, "SeriesTimerCreated", onSeriesTimerCreated), events.off(serverNotifications, "TimerCancelled", onTimerCancelled), events.off(serverNotifications, "SeriesTimerCancelled", onSeriesTimerCancelled), clearCurrentTimeUpdateInterval(), setScrollEvents(options.element, !1), itemShortcuts.off(options.element), items = {}
        };
        var currentTimeUpdateInterval, currentTimeIndicatorBar, currentTimeIndicatorArrow, lastGridScroll = 0,
            lastHeaderScroll = 0;
        require(["text!./tvguide.template.html"], function(template) {
            var context = options.element;
            if (context.classList.add("tvguide"), context.innerHTML = globalize.translateDocument(template, "sharedcomponents"), layoutManager.desktop)
                for (var guideScrollers = context.querySelectorAll(".guideScroller"), i = 0, length = guideScrollers.length; i < length; i++) guideScrollers[i].classList.add("darkScroller");
            var programGrid = context.querySelector(".programGrid"),
                timeslotHeaders = context.querySelector(".timeslotHeaders");
            layoutManager.tv && programGrid.addEventListener("focus", onProgramGridFocus, !0), (browser.iOS || browser.osx) && (context.querySelector(".channelsContainer").classList.add("noRubberBanding"), programGrid.classList.add("noRubberBanding")), dom.addEventListener(programGrid, "scroll", function(e) {
                ...
                currentStartIndex = 0, reloadPage(context), restartAutoRefresh()
                ...
                if (tabButton) {
                    ...
                    var scrollToTimeMs, programGrid = context.querySelector(".programGrid"),
                        scrollWidth = programGrid.scrollWidth;
                    scrollToTimeMs = scrollWidth ? programGrid.scrollLeft / scrollWidth * msPerDay : 0, changeDate(context, date, scrollToTimeMs, scrollToTimeMs, !1)
                }
