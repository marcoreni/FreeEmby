define(["browser", "appStorage", "apphost", "loading", "connectionManager", "globalize", "embyRouter", "dom", "css!./multiselect"], function(browser, appStorage, appHost, loading, connectionManager, globalize, embyRouter, dom) {
    ...
    function updateItemSelection(chkItemSelect, selected) {
        ...
        if (selected) {
            ...
            current.length || (selectedItems.push(id), selectedElements.push(chkItemSelect))
    ...
    function showSelectionCommands() {
        ...
        if (!selectionCommandsPanel) {
            ...
            html += '<button is="paper-icon-button-light" class="btnSelectionPanelOptions autoSize" style="margin-left:auto;"><i class="md-icon">' + moreIcon + "</i></button>", selectionCommandsPanel.innerHTML = html, selectionCommandsPanel.querySelector(".btnCloseSelectionPanel").addEventListener("click", hideSelections);
    ...
    function showMenuForSelectedItems(e) {
        ...
        apiClient.getCurrentUser().then(function(user) {
            ...
            menuItems.push({
                ...
                id: "delete",
                ironIcon: "delete"
            }), user.Policy.EnableContentDownloading && appHost.supports("filedownload"), menuItems.push({
                name: globalize.translate("sharedcomponents#GroupVersions"),
                id: "groupvideos",
                ironIcon: "call-merge"
            }), user.Policy.EnableSync && appHost.supports("sync") && menuItems.push({
                ...
                id: "refresh"
            }), user.Policy.EnableSync && menuItems.push({
    ...
    function dispatchNeedsRefresh() {
        ...
        [].forEach.call(selectedElements, function(i) {
            ...
            container && elems.indexOf(container) === -1 && elems.push(container)
        });
        for (var i = 0, length = elems.length; i < length; i++) elems[i].dispatchEvent(new CustomEvent("needsrefresh", {
            detail: {},
            cancelable: !1,
            bubbles: !0
        }))
    ...
    function combineVersions(apiClient, selection) {
        return selection.length < 2 ? void require(["alert"], function(alert) {
            alert({
                text: globalize.translate("sharedcomponents#PleaseSelectTwoItems")
            })
        }) : (loading.show(), void apiClient.ajax({
            type: "POST",
    ...
    return document.addEventListener("viewbeforehide", hideSelections),
        function(options) {
            ...
            initTapHold(container), options.bindOnClick !== !1 && container.addEventListener("click", onContainerClick), self.onContainerClick = onContainerClick, self.destroy = function() {
