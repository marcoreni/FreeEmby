define(["itemShortcuts", "connectionManager", "layoutManager", "browser", "dom", "loading", "focusManager", "serverNotifications", "events", "registerElement"], function(itemShortcuts, connectionManager, layoutManager, browser, dom, loading, focusManager, serverNotifications, events) {
    ...
    function onClick(e) {
        ...
        multiSelect && multiSelect.onContainerClick.call(itemsContainer, e) === !1 || itemShortcuts.onClick.call(itemsContainer, e)
    }
    ...
    function onContextMenu(e) {
        ...
        if (card && card.getAttribute("data-serverid")) return itemShortcuts.showContextMenu(card, {
            positionTo: target,
    ...
    function onDrop(evt, itemsContainer) {
        ...
        newIndex = Math.max(0, newIndex - 1), loading.show(), apiClient.ajax({
            url: apiClient.getUrl("Playlists/" + playlistId + "/Items/" + itemId + "/Move/" + newIndex),
            type: "POST"
        }).then(function() {
            loading.hide()
        }, function() {
            loading.hide(), itemsContainer.dispatchEvent(new CustomEvent("needsrefresh", {
                detail: {},
                cancelable: !1,
                bubbles: !0
            }))
    ...
    function removeNotificationEvent(instance, name) {
        ...
        handler && (events.off(serverNotifications, "UserDataChanged", handler), instance[name] = null)
    }
    ...
    function onKeyDown(e) {
        ...
        chr = alphanumeric(chr), chr && (currentDisplayTextContainer = this, onAlphanumericKeyPress(e, chr))
    }
    ...
    ItemsContainerProtoType.enableHoverMenu = function(enabled) {
        ...
        if (!enabled) return void(current && (current.destroy(), this.hoverMenu = null));
        if (!current) {
            var self = this;
