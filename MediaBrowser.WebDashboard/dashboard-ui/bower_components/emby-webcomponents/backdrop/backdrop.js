define(["browser", "connectionManager", "playbackManager", "dom", "css!./style"], function(browser, connectionManager, playbackManager, dom) {
    ...
    function clearBackdrop(clearAll) {
        clearRotation(), currentLoadingBackdrop && (currentLoadingBackdrop.destroy(), currentLoadingBackdrop = null);
        var elem = getBackdropContainer();
        elem.innerHTML = "", clearAll && (hasExternalBackdrop = !1), internalBackdrop(!1)
    }
    ...
    function getBackdropMaxWidth() {
        ...
        if (standardWidths.indexOf(width) !== -1) return width;
        var roundScreenTo = 100;
        return width = Math.floor(width / roundScreenTo) * roundScreenTo, Math.min(width, 1920)
    }
    ...
    function startRotation(images, enableImageRotation) {
        arraysEqual(images, currentRotatingImages) || (clearRotation(), currentRotatingImages = images, currentRotationIndex = -1, images.length > 1 && enableImageRotation !== !1 && enableRotation() && (rotationInterval = setInterval(onRotationInterval, 24e3)), onRotationInterval())
    }
