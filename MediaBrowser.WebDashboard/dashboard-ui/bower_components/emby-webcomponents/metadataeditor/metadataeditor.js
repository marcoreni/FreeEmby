define(["itemHelper", "dom", "layoutManager", "dialogHelper", "datetime", "loading", "focusManager", "connectionManager", "globalize", "require", "shell", "emby-checkbox", "emby-input", "emby-select", "listViewStyle", "emby-textarea", "emby-button", "paper-icon-button-light", "css!./../formdialog"], function(itemHelper, dom, layoutManager, dialogHelper, datetime, loading, focusManager, connectionManager, globalize, require, shell) {
    ...
    function onSubmit(e) {
        ...
        try {
            var item = {
                ...
                Studios: editableListViewValues(form.querySelector("#listStudios")).map(function(element) {
                    return {
                        Name: element
                    }
                }),
            ...
            item.Taglines = tagline ? [tagline] : [], submitUpdatedItem(form, item)
        } catch (err) {
            alert(err)
        }
        ...
        require(["prompt"], function(prompt) {
            prompt({
                ...
                var list = dom.parentWithClass(source, "editableListviewContainer").querySelector(".paperList"),
                    items = editableListViewValues(list);
    ...
    function editPerson(context, person, index) {
        require(["personEditor"], function(personEditor) {
            personEditor.show(person).then(function(updatedPerson) {
                var isNew = index === -1;
                isNew && currentItem.People.push(updatedPerson), populatePeople(context, currentItem.People)
            })
        })
    }
    ...
    function afterDeleted(context, item) {
        ...
        parentId ? reload(context, parentId, item.ServerId) : require(["embyRouter"], function(embyRouter) {
            embyRouter.goHome()
        })
    }
    ...
    function onWebSocketMessageReceived(e, data) {
        var msg = data;
        "LibraryChanged" === msg.MessageType && msg.Data.ItemsUpdated.indexOf(currentItem.Id) !== -1 && (console.log("Item updated - reloading metadata"), reload(currentContext, currentItem.Id, currentItem.ServerId))
    }
    ...
    function onEditorClick(e) {
        ...
        if (btnRemoveFromEditorList) return void removeElementFromListview(btnRemoveFromEditorList);
        var btnAddTextItem = dom.parentWithClass(e.target, "btnAddTextItem");
        btnAddTextItem && addElementToEditableListview(btnAddTextItem)
    }
    ...
    function init(context, apiClient) {
        ...
        }), isDialog() && bindItemChanged(context, apiClient)
    }
    ...
    function loadExternalIds(context, item, externalIds) {
        for (var html = "", providerIds = item.ProviderIds || {}, i = 0, length = externalIds.length; i < length; i++) {
            ...
            html += '<div class="inputContainer">', html += '<div style="display: flex; align-items: center;">';
            var value = providerIds[idInfo.Key] || "";
            html += '<div style="flex-grow:1;">', html += '<input is="emby-input" class="txtExternalId" value="' + value + '" data-providerkey="' + idInfo.Key + '" data-formatstring="' + formatString + '" id="' + id + '" label="' + labelText + '"/>', html += "</div>", formatString && (html += '<button type="button" is="paper-icon-button-light" class="btnOpenExternalId" data-fieldid="' + id + '"><i class="md-icon">open_in_browser</i></button>'), html += "</div>", html += "</div>"
        }
    ...
    function fillItemInfo(context, item, parentalRatingOptions) {
        ...
        populateStatus(selectStatus), selectStatus.value = item.Status || "", context.querySelector("#select3dFormat", context).value = item.Video3DFormat || "", Array.prototype.forEach.call(context.querySelectorAll(".chkAirDay", context), function(el) {
            ...
            return element.Name || ""
        })), populateListView(context.querySelector("#listTags"), item.Tags), populateListView(context.querySelector("#listKeywords"), item.Keywords);
        ...
        chkLockData.checked = lockData, chkLockData.checked ? hideElement(".providerSettingsContainer", context) : showElement(".providerSettingsContainer", context), populateInternetProviderSettings(context, item, item.LockedFields), context.querySelector("#txtPath").value = item.Path || "", context.querySelector("#txtName").value = item.Name || "", context.querySelector("#txtOriginalName").value = item.OriginalTitle || "", context.querySelector("#txtOverview").value = item.Overview || "", context.querySelector("#txtTagline").value = item.Taglines && item.Taglines.length ? item.Taglines[0] : "", context.querySelector("#txtSortName").value = item.ForcedSortName || "", context.querySelector("#txtDisplayMediaType").value = item.DisplayMediaType || "", context.querySelector("#txtCommunityRating").value = item.CommunityRating || "", context.querySelector("#txtCommunityVoteCount").value = item.VoteCount || "", context.querySelector("#txtHomePageUrl").value = item.HomePageUrl || "", context.querySelector("#txtAwardSummary").value = item.AwardSummary || "", context.querySelector("#txtCriticRating").value = item.CriticRating || "", context.querySelector("#txtCriticRatingSummary").value = item.CriticRatingSummary || "", context.querySelector("#txtIndexNumber").value = "IndexNumber" in item ? item.IndexNumber : "", context.querySelector("#txtParentIndexNumber").value = "ParentIndexNumber" in item ? item.ParentIndexNumber : "", context.querySelector("#txtPlayers").value = item.Players || "", context.querySelector("#txtAbsoluteEpisodeNumber").value = "AbsoluteEpisodeNumber" in item ? item.AbsoluteEpisodeNumber : "", context.querySelector("#txtDvdEpisodeNumber").value = "DvdEpisodeNumber" in item ? item.DvdEpisodeNumber : "", context.querySelector("#txtDvdSeasonNumber").value = "DvdSeasonNumber" in item ? item.DvdSeasonNumber : "", context.querySelector("#txtAirsBeforeSeason").value = "AirsBeforeSeasonNumber" in item ? item.AirsBeforeSeasonNumber : "", context.querySelector("#txtAirsAfterSeason").value = "AirsAfterSeasonNumber" in item ? item.AirsAfterSeasonNumber : "", context.querySelector("#txtAirsBeforeEpisode").value = "AirsBeforeEpisodeNumber" in item ? item.AirsBeforeEpisodeNumber : "", context.querySelector("#txtAlbum").value = item.Album || "", context.querySelector("#txtAlbumArtist").value = (item.AlbumArtists || []).map(function(a) {
            return a.Name
        }).join(";"), context.querySelector("#selectDisplayOrder").value = item.DisplayOrder, context.querySelector("#txtArtist").value = (item.ArtistItems || []).map(function(a) {
            return a.Name
        }).join(";");
        var date;
    ...
    function populateListView(list, items, sortCallback) {
        items = items || [], "undefined" == typeof sortCallback ? items.sort(function(a, b) {
    ...
    function populatePeople(context, people) {
        for (var lastType = "", html = "", elem = context.querySelector("#peopleList"), i = 0, length = people.length; i < length; i++) {
            var person = people[i];
            html += '<div class="listItem">', html += '<i class="md-icon listItemIcon" style="background-color:#333;">person</i>', html += '<div class="listItemBody">', html += '<a class="btnEditPerson clearLink" href="#" data-index="' + i + '">', html += '<div class="textValue">', html += person.Name || "", html += "</div>", person.Role && person.Role !== lastType && (html += '<div class="secondary">' + person.Role + "</div>"), html += "</a>", html += "</div>", html += '<button type="button" is="paper-icon-button-light" data-index="' + i + '" class="btnDeletePerson autoSize"><i class="md-icon">delete</i></button>', html += "</div>"
        }
        elem.innerHTML = html
    }
    ...
    function generateSliders(fields, currentFields) {
        for (var html = "", i = 0; i < fields.length; i++) {
            var field = fields[i],
                ...
                checkedHtml = currentFields.indexOf(value) === -1 ? " checked" : "";
    ...
    function populateInternetProviderSettings(context, item, lockedFields) {
        ...
        "Person" === item.Type ? metadatafields.push({
            ...
            value: "ProductionLocations"
        }), "Series" === item.Type && metadatafields.push({
            ...
            name: globalize.translate("sharedcomponents#Keywords"),
            value: "Keywords"
        }), metadatafields.push({
            name: globalize.translate("sharedcomponents#Images"),
            value: "Images"
        }), metadatafields.push({
            name: globalize.translate("sharedcomponents#Backdrops"),
            value: "Backdrops"
        }), "Game" === item.Type && metadatafields.push({
            name: globalize.translate("sharedcomponents#Screenshots"),
            value: "Screenshots"
        });
        var html = "";
        html += "<h1>" + globalize.translate("sharedcomponents#HeaderEnabledFields") + "</h1>", html += "<p>" + globalize.translate("sharedcomponents#HeaderEnabledFieldsHelp") + "</p>", html += generateSliders(metadatafields, lockedFields), container.innerHTML = html
    }
    ...
    function show(itemId, serverId, resolve, reject) {
        loading.show(), require(["text!./metadataeditor.template.html"], function(template) {
            ...
            html += globalize.translateDocument(template, "sharedcomponents"), dlg.innerHTML = html, layoutManager.tv && centerFocus(dlg.querySelector(".formDialogContent"), !1, !0), dialogHelper.open(dlg), dlg.addEventListener("close", function() {
                layoutManager.tv && centerFocus(dlg.querySelector(".formDialogContent"), !1, !1), unbindItemChanged(dlg, connectionManager.getApiClient(serverId)), resolve()
