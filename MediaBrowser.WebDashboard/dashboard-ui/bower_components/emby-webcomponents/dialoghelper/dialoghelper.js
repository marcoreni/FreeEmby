define(["historyManager", "focusManager", "browser", "layoutManager", "inputManager", "dom", "css!./dialoghelper.css", "scrollStyles"], function(historyManager, focusManager, browser, layoutManager, inputManager, dom) {
    ...
    function enableAnimation() {
        return !browser.tv && (!browser.noAnimation && browser.supportsCssAnimation())
    }

    function removeCenterFocus(dlg) {
        layoutManager.tv && (dlg.classList.contains("smoothScrollX") ? centerFocus(dlg, !0, !1) : dlg.classList.contains("smoothScrollY") && centerFocus(dlg, !1, !1))
    }
    ...
    function DialogHashHandler(dlg, hash, resolve) {
        ...
        function onDialogClosed() {
            if (isHistoryEnabled(dlg) || inputManager.off(dlg, onBackCommand), window.removeEventListener("popstate", onHashChange), removeBackdrop(dlg), dlg.classList.remove("opened"), removeScrollLockOnClose && document.body.classList.remove("noScroll"), !self.closedByBack && isHistoryEnabled(dlg)) {
                ...
                state.dialogId === hash && history.back()
            }
            if (layoutManager.tv && activeElement.focus(), "false" !== dlg.getAttribute("data-removeonclose")) {
        ...
        center && dlg.classList.add("centeredDialog"), dlg.classList.remove("hide"), addBackdropOverlay(dlg), dlg.classList.add("opened"), dlg.dispatchEvent(new CustomEvent("open", {
            bubbles: !1,
            cancelable: !1
        })), "true" !== dlg.getAttribute("data-lockscroll") || document.body.classList.contains("noScroll") || (document.body.classList.add("noScroll"), removeScrollLockOnClose = !0), animateDialogOpen(dlg), isHistoryEnabled(dlg) ? (historyManager.pushState({
            dialogId: hash
    ...
    function addBackdropOverlay(dlg) {
        ...
        backdropParent.parentNode.insertBefore(backdrop, backdropParent), dlg.backdrop = backdrop, void backdrop.offsetWidth, backdrop.classList.add("dialogBackdropOpened"), dom.addEventListener(dlg.dialogContainer || backdrop, "click", function(e) {
    ...
    function shouldLockDocumentScroll(options) {
        return null != options.lockScroll ? options.lockScroll : "fullscreen" === options.size || (!!options.size || browser.touch)
    }
    ...
    function removeBackdrop(dlg) {
        ...
        if (backdrop) {
            ...
            return enableAnimation() ? (backdrop.classList.remove("dialogBackdropOpened"), void setTimeout(onAnimationFinish, 300)) : void onAnimationFinish()
        }
    }
    ...
    function createDialog(options) {
        ...
        var defaultEntryAnimation = "scaleup",
            entryAnimation = options.entryAnimation || defaultEntryAnimation,
            ...
            exitAnimationDuration = options.exitAnimationDuration || ("fullscreen" !== options.size ? 180 : 280);
        if (dlg.animationConfig = {
            ...
            }, dlg.classList.add("dialog"), options.scrollX ? (dlg.classList.add("smoothScrollX"), layoutManager.tv && centerFocus(dlg, !0, !0)) : options.scrollY !== !1 && (dlg.classList.add("smoothScrollY"), layoutManager.tv && centerFocus(dlg, !1, !0)), options.removeOnClose && dlg.setAttribute("data-removeonclose", "true"), options.size && (dlg.classList.add("dialog-fixedSize"), dlg.classList.add("dialog-" + options.size)), enableAnimation()) switch (dlg.animationConfig.entry.name) {
            case "fadein":
