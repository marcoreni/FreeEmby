define(["loading", "viewManager", "skinManager", "pluginManager", "backdrop", "browser", "pageJs", "appSettings", "apphost", "connectionManager"], function(loading, viewManager, skinManager, pluginManager, backdrop, browser, page, appSettings, appHost, connectionManager) {
        ...
        switch (result.State) {
            ...
            case MediaBrowser.ConnectionState.ServerSignIn:
                result.ApiClient.getPublicUsers().then(function(users) {
                    users.length ? embyRouter.showLocalLogin(result.ApiClient, result.Servers[0].Id) : embyRouter.showLocalLogin(result.ApiClient, result.Servers[0].Id, !0)
                });
                break;
    ...
    function loadContentUrl(ctx, next, route, request) {
        ...
        url.indexOf("://") === -1 && (0 !== url.indexOf("/") && (url = "/" + url), url = baseUrl() + url), ctx.querystring && route.enableContentQueryString && (url += "?" + ctx.querystring), require(["text!" + url], function(html) {
    ...
    function sendRouteToViewManager(ctx, next, route, controllerFactory) {
        ...
        return isBackNav ? void viewManager.tryRestoreView(currentRequest, function() {
            currentRouteInfo = {
                route: route,
                path: ctx.path
            }
    ...
    function start(options) {
        loading.show(), connectionManager.connect({
            ...
            firstConnectionResult = result, loading.hide(), options = options || {}, page({
                click: options.click !== !1,
                hashbang: options.hashbang !== !1,
    ...
    function enableHistory() {
        return !browser.xboxOne && !browser.edgeUwp && !browser.orsay
    }
    ...
    function authenticate(ctx, route, callback) {
        ...
        if (firstResult && (firstConnectionResult = null, firstResult.State !== MediaBrowser.ConnectionState.SignedIn && !route.anonymous)) return void handleConnectionResult(firstResult, loading);
        ...
        if (!(shouldExitApp || apiClient && apiClient.isLoggedIn() || route.anonymous)) return console.log("embyRouter - route does not allow anonymous access, redirecting to login"), void beginConnectionWizard();
        if (shouldExitApp) {
            if (appHost.supports("exit")) return void appHost.exit()
        } else {
            if (apiClient && apiClient.isLoggedIn()) {
                if (console.log("embyRouter - user is authenticated"), ctx.isBack && (route.isDefaultRoute || route.startup) && !isCurrentRouteStartup) return void handleBackToDefault();
                if (route.isDefaultRoute) return console.log("embyRouter - loading skin home page"), void skinManager.loadUserSkin();
                if (route.roles) return void validateRoles(apiClient, route.roles).then(function() {
                    apiClient.ensureWebSocket(), callback()
                }, beginConnectionWizard)
            }
            console.log("embyRouter - proceeding to " + pathname), callback()
        }
    }
    ...
    function handleBackToDefault() {
        return !appHost.supports("exitmenu") && appHost.supports("exit") ? void appHost.exit() : (isDummyBackToHome = !0, skinManager.loadUserSkin(), void(isHandlingBackToDefault || skinManager.getCurrentSkin().showBackMenu().then(function() {
    ...
    function getRequestFile() {
        ...
        return path = index !== -1 ? path.substring(index) : "/" + path, path && "/" !== path || (path = "/index.html"), path
    }
    ...
    function getWindowLocationSearch(win) {
        var currentPath = currentRouteInfo ? currentRouteInfo.path || "" : "",
            ...
            search = "";
        return index !== -1 && (search = currentPath.substring(index)), search || ""
    }
    ...
    function show(path, options) {
        0 !== path.indexOf("/") && path.indexOf("://") === -1 && (path = "/" + path);
    ...
    function showItem(item, serverId, options) {
        ...
        } else 2 === arguments.length && (options = arguments[1]), skinManager.getCurrentSkin().showItem(item, options)
    }
    ...
    var currentViewLoadRequest, firstConnectionResult, isHandlingBackToDefault, isDummyBackToHome, embyRouter = {
        ...
        baseRoute = window.location.href.split("?")[0].replace(getRequestFile(), "");
    baseRoute = baseRoute.split("#")[0], baseRoute.lastIndexOf("/") === baseRoute.length - 1 && (baseRoute = baseRoute.substring(0, baseRoute.length - 1));
