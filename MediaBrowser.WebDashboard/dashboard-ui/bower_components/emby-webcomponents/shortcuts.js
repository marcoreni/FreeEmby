define(["playbackManager", "inputManager", "connectionManager", "embyRouter", "globalize", "loading", "dom", "recordingHelper"], function(playbackManager, inputManager, connectionManager, embyRouter, globalize, loading, dom, recordingHelper) {
    ...
    function playAllFromHere(card, serverId, queue) {
        for (var parent = card.parentNode, className = card.classList.length ? "." + card.classList[0] : "", cards = parent.querySelectorAll(className + "[data-id]"), ids = [], foundCard = !1, i = 0, length = cards.length; i < length; i++) cards[i] === card && (foundCard = !0), foundCard && ids.push(cards[i].getAttribute("data-id"));
        ids.length && (queue ? playbackManager.queue({
            ids: ids,
            serverId: serverId
    ...
    function showItem(item, options) {
        return "Photo" === item.Type ? void showSlideshow(item.Id, item.ServerId) : void embyRouter.showItem(item, options)
    }
            ...
            require(["itemContextMenu"], function(itemContextMenu) {
                connectionManager.getApiClient(item.ServerId).getCurrentUser().then(function(user) {
                    itemContextMenu.show(Object.assign({
                        ...
                        var itemsContainer;
                        "playallfromhere" === result.command || "queueallfromhere" === result.command ? executeAction(card, options.positionTo, result.command) : "removefromplaylist" === result.command || "removefromcollection" === result.command ? (itemsContainer = options.itemsContainer || dom.parentWithAttribute(card, "is", "emby-itemscontainer"), itemsContainer && itemsContainer.dispatchEvent(new CustomEvent("needsrefresh", {
                            detail: {},
                            cancelable: !1,
                            bubbles: !0
                        }))) : "canceltimer" === result.command && (itemsContainer = options.itemsContainer || dom.parentWithAttribute(card, "is", "emby-itemscontainer"), itemsContainer && itemsContainer.dispatchEvent(new CustomEvent("timercancelled", {
                            detail: {},
                            cancelable: !1,
                            bubbles: !0
                        })))
    ...
    function onClick(e) {
        ...
        if (card) {
            ...
            if (action || (actionElement = dom.parentWithAttribute(actionElement, "data-action"), actionElement && (action = actionElement.getAttribute("data-action"))), action) return executeAction(card, actionElement, action), e.preventDefault(), e.stopPropagation(), !1
        }
    }
    ...
    function on(context, options) {
        options = options || {}, options.click !== !1 && context.addEventListener("click", onClick), options.command !== !1 && inputManager.on(context, onCommand)
    }

    function off(context, options) {
        options = options || {}, context.removeEventListener("click", onClick), options.command !== !1 && inputManager.off(context, onCommand)
    }
