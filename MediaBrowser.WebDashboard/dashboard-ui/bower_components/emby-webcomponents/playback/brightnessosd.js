define(["events", "playbackManager", "dom", "browser", "css!./iconosd", "material-icons"], function(events, playbackManager, dom, browser) {
    ...
    function hideOsd() {
        clearHideTimeout();
        var elem = osdElement;
        elem && (enableAnimation ? (void elem.offsetWidth, requestAnimationFrame(function() {
    ...
    function updateElementsFromPlayer(brightness) {
        iconElement && (brightness >= 80 ? iconElement.innerHTML = "&#xE1AC;" : brightness >= 20 ? iconElement.innerHTML = "&#xE1AE;" : iconElement.innerHTML = "&#xE1AD;"), progressElement && (progressElement.style.width = (brightness || 0) + "%")
    }
