define(["connectionManager", "globalize", "dom", "itemHelper", "paper-icon-button-light", "material-icons", "emby-button", "css!./userdatabuttons"], function(connectionManager, globalize, dom, itemHelper) {
    ...
    function getIconsHtml(options) {
        ...
        if (includePlayed !== !1) {
            ...
            "Video" !== item.MediaType && "Series" !== item.Type && "Season" !== item.Type && "BoxSet" !== item.Type && "Playlist" !== item.Type || "TvChannel" !== item.Type && (html += userData.Played ? getUserDataButtonHtml("markPlayed", itemId, serverId, btnCssClass + " btnUserDataOn", iconCssClass, "&#xE5CA;", tooltipPlayed, style) : getUserDataButtonHtml("markPlayed", itemId, serverId, btnCssClass, iconCssClass, "&#xE5CA;", tooltipPlayed, style))
        }
    ...
    function played(id, serverId, isPlayed) {
        var apiClient = connectionManager.getApiClient(serverId),
            method = isPlayed ? "markPlayed" : "markUnplayed";
