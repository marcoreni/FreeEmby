define(["appSettings", "loading", "apphost", "iapManager", "events", "shell", "globalize", "dialogHelper", "connectionManager", "layoutManager", "emby-button"], function(appSettings, loading, appHost, iapManager, events, shell, globalize, dialogHelper, connectionManager, layoutManager) {
    ...
    function showPeriodicMessageIfNeeded(feature) {
        ...
        return lastMessage ? (new Date).getTime() - lastMessage > intervalMs ? connectionManager.currentApiClient().getPluginSecurityInfo().then(function(regInfo) {
            return regInfo.IsMBSupporter ? (appSettings.set(settingsKey, (new Date).getTime()), Promise.resolve()) : showPeriodicMessage(feature, settingsKey)
        }, function() {
            return showPeriodicMessage(feature, settingsKey)
        }) : Promise.resolve() : (appSettings.set(settingsKey, (new Date).getTime()), Promise.resolve())
    }
        ...
        return options = options || {}, console.log("validateFeature: " + feature), iapManager.isUnlockedByDefault(feature, options).then(function() {
            ...
            return iapManager.getSubscriptionOptions().then(function(subscriptionOptions) {
                return subscriptionOptions.filter(function(p) {
                    ...
                    return options.showDialog === !1 ? Promise.reject() : showInAppPurchaseInfo(subscriptionOptions, unlockableProductInfo, dialogOptions)
                })
            })
        })
    }
    ...
    function clearCurrentDisplayingInfo() {
        currentDisplayingProductInfos = [], currentDisplayingResolve = null
    }
    ...
    function onProductUpdated(e, product) {
        if (product.owned) {
            ...
            resolve && currentDisplayingProductInfos.filter(function(p) {
                return product.id === p.id
            }).length && (cancelInAppPurchase(), resolve())
        }
    }
