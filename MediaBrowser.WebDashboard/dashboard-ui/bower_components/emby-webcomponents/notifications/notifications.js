define(["serverNotifications", "playbackManager", "events", "globalize", "require"], function(serverNotifications, playbackManager, events, globalize, require) {
    ...
    function showNotification(options, timeoutMs, apiClient) {
        ...
        return options.data = options.data || {}, options.data.serverId = apiClient.serverInfo().Id, options.icon = options.icon || getIconUrl(), options.badge = options.badge || getIconUrl("badge.png"), resetRegistration(), serviceWorkerRegistration ? void showPersistentNotification(title, options, timeoutMs) : void showNonPersistentNotification(title, options, timeoutMs)
    }
    ...
    function onLibraryChanged(data, apiClient) {
        ...
        newItems.length && apiClient.getItems(apiClient.getCurrentUserId(), {
            Recursive: !0,
            Limit: 3,
            Filters: "IsNotFolder",
            SortBy: "DateCreated",
    ...
    function showPackageInstallNotification(apiClient, installation, status) {
        apiClient.getCurrentUser().then(function(user) {
            if (user.Policy.IsAdministrator) {
                ...
                var timeout = "cancelled" === status ? 5e3 : 0;
