define(["serverNotifications", "events", "loading", "connectionManager", "imageLoader", "dom", "globalize", "registrationServices", "layoutManager", "listViewStyle"], function(serverNotifications, events, loading, connectionManager, imageLoader, dom, globalize, registrationServices, layoutManager) {
    ...
    function cancelJob(listInstance, id) {
        require(["confirm"], function(confirm) {
            var msg = listInstance.options.isLocalSync ? globalize.translate("sharedcomponents#ConfirmRemoveDownload") : globalize.translate("sharedcomponents#CancelSyncJobConfirmation");
            confirm({
                text: msg,
                primary: "cancel"
            }).then(function() {
    ...
    function getSyncJobHtml(listInstance, job, apiClient) {
        ...
        layoutManager.tv && (listItemClass += " listItem-button listItem-focusscale", listItemClass += " btnJobMenu"), html += "<" + tagName + typeAttribute + ' class="' + listItemClass + '" data-id="' + job.Id + '" data-status="' + job.Status + '">';
        var imgUrl;
        ...
        return html += '<div class="secondary listItemBodyText jobStatus" style="color:green;">', html += getProgressText(job), html += "</div>", html += "</div>", layoutManager.tv || (html += '<button type="button" is="paper-icon-button-light" class="btnJobMenu listItemButton"><i class="md-icon">more_vert</i></button>'), html += "</" + tagName + ">"
    }
    ...
    function renderList(listInstance, jobs, apiClient) {
        ...
        for (var html = "", lastTargetName = "", isLocalSync = listInstance.options.isLocalSync, showTargetName = !isLocalSync, hasOpenSection = !1, i = 0, length = jobs.length; i < length; i++) {
            var job = jobs[i];
            if (showTargetName) {
                var targetName = job.TargetName || "Unknown";
                targetName !== lastTargetName && (lastTargetName && (html += "</div>", html += "<br/>", hasOpenSection = !1), lastTargetName = targetName, html += '<div class="detailSectionHeader">', html += "<h1>" + targetName + "</h1>", html += "</div>", html += '<div class="itemsContainer vertical-list paperList">', hasOpenSection = !0)
            }
            html += getSyncJobHtml(listInstance, job, apiClient)
        }
        hasOpenSection && (html += "</div>");
        var elem = listInstance.options.element.querySelector(".syncJobListContent");
        html || (html = isLocalSync ? '<div style="padding:1em .25em;">' + globalize.translate("sharedcomponents#MessageNoDownloadsFound") + "</div>" : '<div style="padding:1em .25em;">' + globalize.translate("sharedcomponents#MessageNoSyncJobsFound") + "</div>"), elem.innerHTML = html, imageLoader.lazyChildren(elem)
    }
    ...
    function fetchData(listInstance) {
        ...
        return listInstance.options.userId && (options.UserId = listInstance.options.userId), listInstance.options.isLocalSync ? options.TargetId = apiClient.deviceId() : options.ExcludeTargetIds = apiClient.deviceId(), apiClient.getJSON(apiClient.getUrl("Sync/Jobs", options)).then(function(response) {
    ...
    function startListening(listInstance) {
        var startParams = "0,1500",
            apiClient = getApiClient(listInstance);
        listInstance.options.userId && (startParams += "," + listInstance.options.userId), listInstance.options.isLocalSync && (startParams += "," + apiClient.deviceId()), apiClient.isWebSocketOpen() && apiClient.sendWebSocketMessage("SyncJobsStart", startParams)
    }

    function stopListening(listInstance) {
        var apiClient = getApiClient(listInstance);
        apiClient.isWebSocketOpen() && apiClient.sendWebSocketMessage("SyncJobsStop", "")
    }
    ...
    function showJobMenu(listInstance, elem) {
        var item = dom.parentWithClass(elem, "listItem"),
            ...
            menuItems = [];
        if ("Cancelled" === status) menuItems.push({
            ...
            var txt = listInstance.options.isLocalSync ? globalize.translate("sharedcomponents#RemoveDownload") : globalize.translate("sharedcomponents#ButtonCancelSyncJob");
            menuItems.push({
                name: txt,
                id: "cancel"
            })
    ...
    function onElementClick(e) {
        ...
        return btnJobMenu ? void showJobMenu(this, btnJobMenu) : void showJobEditor(listInstance, e.target)
    }
    ...
    function syncJobList(options) {
        ...
        this.onSyncJobsUpdatedHandler = onSyncJobsUpdatedHandler, events.on(serverNotifications, "SyncJobs", onSyncJobsUpdatedHandler);
        var onClickHandler = onElementClick.bind(this);
        options.element.addEventListener("click", onClickHandler), this.onClickHandler = onClickHandler, options.element.innerHTML = '<div class="syncJobListContent"></div>', fetchData(this), startListening(this), initSupporterInfo(options.element, getApiClient(this))
    }
    ...
    function initSupporterInfo(context, apiClient) {
        apiClient.getPluginSecurityInfo().then(function(regInfo) {
            regInfo.IsMBSupporter || showSupporterInfo(context, apiClient)
        }, function() {
    ...
    return syncJobList.prototype.destroy = function() {
        ...
        this.onSyncJobsUpdatedHandler = null, events.off(serverNotifications, "SyncJobs", onSyncJobsUpdatedHandler);
