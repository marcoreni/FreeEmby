...
! function(global, Promise, undef) {
    ...
    function newContext(contextName) {
        ...
        function takeQueue(anonId) {
            ...
            for (i = 0; i < queue.length; i += 1) {
                ...
                args = queue.shift(), id = args[0], i -= 1, hasProp(defined, id) || hasProp(waiting, id) || (hasProp(deferreds, id) ? main.apply(undef, args) : waiting[id] = args)
            }
        ...
        function makeRequire(relName, topLevel) {
            var req = function(deps, callback, errback, alt) {
                ...
                if (topLevel && takeQueue(), "string" == typeof deps) {
                    ...
                    if (name = makeMap(deps, relName, !0).id, !hasProp(defined, name)) throw new Error("Not loaded: " + name);
                    return defined[name]
                }
            ...
            return req.isBrowser = "undefined" != typeof document && "undefined" != typeof navigator, req.nameToUrl = function(moduleName, ext, skipExt) {
                ...
                return index !== -1 && (!isRelative || index > 1) && (ext = moduleNamePlusExt.substring(index, moduleNamePlusExt.length), moduleNamePlusExt = moduleNamePlusExt.substring(0, index)), req.nameToUrl(normalize(moduleNamePlusExt, relName), ext, !0)
            }, req.defined = function(id) {
                return hasProp(defined, makeMap(id, relName, !0).id)
            }, req.specified = function(id) {
                return id = makeMap(id, relName, !0).id, hasProp(defined, id) || hasProp(deferreds, id)
            }, req
        }
        ...
        function getDefer(name, calculatedMap) {
            var d;
            return name ? (d = hasProp(deferreds, name) && deferreds[name], d || (d = deferreds[name] = makeDefer(name, calculatedMap))) : (d = makeDefer(), requireDeferreds.push(d)), d
        }
        ...
        function check(d) {
            ...
            0 === loadCount && (d ? d.finished || breakCycle(d, {}, {}) : requireDeferreds.length && requireDeferreds.forEach(function(d) {
                breakCycle(d, {}, {})
            })), expired ? (eachProp(deferreds, function(d) {
                d.finished || notFinished.push(d.map.id)
            }), err = new Error("Timeout for modules: " + notFinished), err.requireModules = notFinished, req.onError(err)) : (loadCount || requireDeferreds.length) && (checkingLater || (checkingLater = !0, setTimeout(function() {
        ...
        var req, main, makeMap, callDep, handlers, checkingLater, load, context, defined = {},
            waiting = {},
            config = {
                waitSeconds: 7,
                baseUrl: "./",
        ...
        return load = "function" == typeof importScripts ? function(map) {
            ...
            urlFetched[url] || (urlFetched[url] = !0, script = document.createElement("script"), script.setAttribute("data-requiremodule", id), script.type = config.scriptType || "text/javascript", script.charset = "utf-8", script.async = !0, loadCount += 1, script.addEventListener("load", function() {
                ...
                pathConfig && Array.isArray(pathConfig) && pathConfig.length > 1 ? (script.parentNode.removeChild(script), pathConfig.shift(), d.map = makeMap(id), load(d.map)) : (err = new Error("Load failed: " + id + ": " + script.src), err.requireModules = [id], getDefer(id).reject(err))
            }, !1), script.src = url, document.head.appendChild(script))
        }, callDep = function(map, relName) {
            var args, bundleId, name = map.id,
                shim = config.shim[name];
            if (hasProp(waiting, name)) args = waiting[name], delete waiting[name], main.apply(undef, args);
                ...
                if (map.pr) {
                    if (!(bundleId = getOwn(bundlesMap, name))) return callDep(makeMap(map.pr)).then(function(plugin) {
                        ...
                        return hasProp(calledPlugin, newId) || (calledPlugin[newId] = !0, shim && shim.deps ? req(shim.deps, function() {
            ...
            return parts = splitPrefix(name), prefix = parts[0], name = parts[1], !prefix && hasProp(mapCache, cacheKey) ? mapCache[cacheKey] : (prefix && (prefix = normalize(prefix, relName, applyMap), plugin = hasProp(defined, prefix) && defined[prefix]), prefix ? plugin && plugin.normalize ? (name = plugin.normalize(name, makeNormalize(relName)), prefixNormalized = !0) : name = name.indexOf("!") === -1 ? normalize(name, relName, applyMap) : name : (name = normalize(name, relName, applyMap), parts = splitPrefix(name), prefix = parts[0], name = parts[1], url = req.nameToUrl(name)), result = {
            ...
            if (!name || !hasProp(calledDefine, name)) {
                calledDefine[name] = !0;
                var d = getDefer(name);
                return deps && !Array.isArray(deps) && (factory = deps, deps = []), errback || (hasProp(config, "defaultErrback") ? config.defaultErrback && (errback = config.defaultErrback) : errback = delayedError), errback && d.promise.catch(errback), relName = relName || name, "function" == typeof factory ? (!deps.length && factory.length && (factory.toString().replace(commentRegExp, commentReplace).replace(cjsRequireRegExp, function(match, dep) {
                    deps.push(dep)
            ...
            if (mapCache = {}, cfg.baseUrl && "/" !== cfg.baseUrl.charAt(cfg.baseUrl.length - 1) && (cfg.baseUrl += "/"), "string" == typeof cfg.urlArgs) {
    ...
    "function" != typeof requirejs && (requirejs = topReq = newContext("_"), "function" != typeof require && (require = topReq), topReq.exec = function(text) {
        return eval(text)
    }, topReq.contexts = contexts, define = function() {
        queue.push(slice.call(arguments, 0))
    }, define.amd = {
        jQuery: !0
    }, bootstrapConfig && topReq.config(bootstrapConfig), topReq.isBrowser && !contexts._.config.skipDataMain && (dataMain = document.querySelectorAll("script[data-main]")[0], dataMain = dataMain && dataMain.getAttribute("data-main"), dataMain && (dataMain = dataMain.replace(jsSuffixRegExp, ""), bootstrapConfig && bootstrapConfig.baseUrl || dataMain.indexOf("!") !== -1 || (src = dataMain.split("/"), dataMain = src.pop(), subPath = src.length ? src.join("/") + "/" : "./", topReq.config({
        baseUrl: subPath
    })), topReq([dataMain]))))
